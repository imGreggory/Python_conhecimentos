#Condicionais
#Exercício Python 43: 
#Desenvolva uma lógica que leia o peso e a altura de uma pessoa, calcule seu Índice de Massa Corporal (IMC)
#e mostre seu status, de acordo com a tabela abaixo:
#– IMC abaixo de 18,5: Abaixo do Peso
#– Entre 18,5 e 25: Peso Ideal
#– 25 até 30: Sobrepeso
#– 30 até 40: Obesidade
#– Acima de 40: Obesidade Mórbida

peso = float(input("Escreva seu peso(em Kg): "))
altura = float(input("Escreva sua altura(em Metros): "))
imc = peso/(altura**2)

if imc < 18.5:
    print(f"Seu imc é {imc}, logo você está abaixo do peso")
elif 25 > imc >= 18.5:
    print(f"Seu imc é {imc}, você está no seu peso ideal!")
elif 30 > imc >= 25:
    print(f"Seu imc é {imc}, você está sobrepeso!")
elif 40 > imc >= 30:
    print(f"Seu imc é {imc}, você está Obeso!")
else:
    print(f"Seu imc é {imc}, você está em um caso de obesidade mórbida!") 